...
def basicQ(request):
    ...
    bot = ChatBot('Tarunkolla',
        ...
        'chatterbot.logic.MathematicalEvaluation',
            ...
            'threshold': 0.70,
            'default_response': '- I am sorry, but I don\'t know how to answer that. You should probably ask Tarun about it'
        }
    ])
    bot.set_trainer(ListTrainer)
    if not exists:
        for each in os.listdir('convo'):
            chats = open('convo/'+each, 'r').readlines()
            bot.train(chats)
            exists = True
            continue
    if exists:
...
if __name__ == "__main__":
    ...
    # Interactive mode
    colorama.init()
    print("\n\nStarting interacting with FISH Bot:")
    ...
    while True:
        ...
        regextk = re.compile(r'(tarun|kolla|tarun kolla)')
        regexmath = re.compile(r'(\S?\d+\s*[-+/*]\s*\d+\s*[?. ])')
        question = input("\n> ")
        #answers = inference_internal(question)[0]
        if((question == "<unk>") or (question == "newlinechar") or (question is None)):
            print("{}- {}{}".format(colorama.Fore.RED, "I can't answer that!", colorama.Fore.RESET))
        elif regextk.search(question):
            print("{}- {}{}".format(colorama.Fore.GREEN, basicQ(question), colorama.Fore.RESET))
        elif regexmath.search(question):
            try:
                print("{}- {}{}".format(colorama.Fore.GREEN, basicQ(question), colorama.Fore.RESET))
            except:
                print("{}- {}{}".format(colorama.Fore.YELLOW, 'Ask me simple questions like 2 + 3?', colorama.Fore.RESET))
        else:
            answers = inference_internal(question)[0]
            if((question is None) or (answers is None)):
                print(colorama.Fore.RED + "Question can't be empty!" + colorama.Fore.RESET)
            else:
                i = answers['best_index']
                #print(answers['answers'][i])
                print("{}- {}{}".format(colorama.Fore.GREEN if answers['scores'][i] == max(answers['scores']) and answers['scores'][i] >= score_settings['bad_response_threshold'] else colorama.Fore.YELLOW if answers['scores'][i] >= score_settings['bad_response_threshold'] else colorama.Fore.RED, answers['answers'][i],colorama.Fore.RESET))
